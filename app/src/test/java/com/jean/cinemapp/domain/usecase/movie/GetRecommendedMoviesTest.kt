...
class GetRecommendedMoviesTest {
    ...
    private val getRecommendedMoviesImpl = GetRecommendedMoviesImpl(movieRepository)
    ...
    fun `get success result from repository`() {
        runBlockingTest {
            given(getRecommendedMoviesImpl.invoke()).willReturn(successResult)

            val result = getRecommendedMoviesImpl.invoke()
    ...
    fun `get error result from repository`() {
        runBlockingTest {
            given(getRecommendedMoviesImpl.invoke()).willReturn(errorResultEmptyList)
    ...
    fun `get error failed response result from repository`() {
        runBlockingTest {
            given(getRecommendedMoviesImpl.invoke()).willReturn(errorResultFailedResponse)
    ...
    fun `get error failed connection result from repository`() {
        runBlockingTest {
            given(getRecommendedMoviesImpl.invoke()).willReturn(errorResultFailedConnection)
    ...
    fun `get error without connection result from repository`() {
        runBlockingTest {
            given(getRecommendedMoviesImpl.invoke()).willReturn(errorResultWithoutConnection)
